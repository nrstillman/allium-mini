...
def calculate_summary_statistics(d, opts = ['A','B','C','D','E','F','G','H'],useall=True, log=False,starttime=60,endtime=320,takeDrift=False, plot = False, usetypes = [1,2],log_output="log.txt"):
    ...
    if 'B' in opts:
        ...
        tval2, velauto = getVelAuto(d, usetype=[1],verbose=plot)
        ssdata['tval2'] = tval2
    ...
    if 'H' in opts:
            ...
            print(f"Stepping Fourier transform with step {dq:.2f}, resulting in {nq} steps (qmax = {qmax}).")
            qrad,valrad,Sqrad = FourierTransVel(d, qmax,whichframe = 0, usetype=[1,2],verbose=plot)
            ssdata['qrad1'] = qrad
            ssdata['valrad1'] = [valrad]
            ssdata['Sqrad1'] = [Sqrad]
            for u in range(1,endtime - starttime):
                print(u,end='\r')
                qrad,valrad,Sqrad = FourierTransVel(d, qmax,whichframe = u, usetype=[1,2],verbose=plot)
    ...
    if 'J' in opts:
        # # J - Division fit
        tvals, Nvals = getDivRate(d, verbose=plot)
        ssdata['tval3'] = tvals
        ssdata['Nvals'] = Nvals
        ...
        if log: print('Finished calculating J. Division details', file=open(log_output, 'a'))
    ...
    if 'K' in opts:
        ...
        tvals, vicsek = getVicsekParam(d, verbose=plot)
        ssdata['tval4'] = tvals
        ssdata['vicsek'] = vicsek
        
        if log: print('Finished calculating K. Vicsek order parameter', file=open(log_output, 'a'))

    if log: print('Finished calculating summary statistics', file=open(log_output, 'a'))
    if useall:
        return combine(ssdata, opts), ssdata
    else:
        return ssvect, ssdata

def combine(ssdata, opts, ss_length = 50):

    features = np.zeros(ss_length*len(opts))
    if 'A' in opts:
        x,y = allium.utils.prep_vav(ssdata)
        features[:50] = y/2
    if 'B' in opts:
        x,y = allium.utils.prep_velautocorr(ssdata)
        features[50:100] = 10*y
    if 'C' in opts:
        x,y = allium.utils.prep_msd(ssdata)
        features[100:150] = y
    if 'H' in opts:
        x,y = allium.utils.prep_fourier_vel(ssdata)
        features[150:200] = 5*y
    if 'D' in opts:
        x,y = allium.utils.prep_selfint(ssdata)
        features[200:250] = 7*y
    if 'F' in opts:
         x,y = allium.utils.prep_gr(ssdata, bounds = [3,50])
         features[250:300] = 5*y
    if 'J' in opts:
         x,y = allium.utils.prep_Nvals(ssdata, bounds = [1,100])
         features[300:350] = y
    if 'K' in opts:
         x,y = allium.utils.prep_vicsek(ssdata, bounds = [3,50])
         features[350:400] = 10*y                  
    return features

def getVicsekParam(data,verbose=True):
    tval = np.linspace(0,(data.Nsnap-1)*data.param.framerate,num=(data.Nsnap-1))
    vicsek_param = []
    for whichframe in range(data.Nsnap-1):   
        # get only points with velocity data
        isdata = data.gettypes([1,2], whichframe)         

        vx = data.vval[whichframe,isdata,0]
        vy = data.vval[whichframe,isdata,1]
        vav = np.sqrt(np.sum(vx**2 + vy**2)/len(isdata))

        vscaled = data.vval[whichframe,isdata] / vav

        vicsek_vect = np.mean(vscaled,axis=0)
        vicsek_param.append(vicsek_vect)

    return  tval, np.array(vicsek_param), 

def getDivRate(data,verbose=True):
    # get time, N values
...
def getVelDist(data,bins,bins2,usetype=[1,2],verbose=True):
    ...
    return vav, vdist, np.sqrt(vdistx**2) + np.sqrt(vdisty**2)


def getMSD(data, usetype=[1],verbose=True):
...
def calcgr(data, usetype=[0,1,2], step=10, verbose = True, periodic=True,resolution=2):
    ...
    max_distance = min(data.param.Lx,data.param.Ly)/8
...
#use all
def FourierTransVel(data,qmax=0.3,whichframe=1,usetype=[0,1],verbose=True, debug=False,L = 800):
    L = data.param.Lx
    # Note to self: only low q values will be interesting in any case. 
    # The stepping is in multiples of the inverse box size. Assuming a square box.
    dq=2*np.pi/L
